...
; Assumes it is entered in real mode with interrupts usable.
...
switch_to_pmode:
    ...
    ; A far jump is required here: it flushes the prefetch queue and
    ; loads CS with the new 32-bit code selector.
    jmp CODE_SEG:pmode_entry

; ---------------------------------------------------------------
; A20 gate - without this, address line 20 stays stuck and memory
; above 1MB wraps around to zero.
; ---------------------------------------------------------------
enable_a20:
    in  al, 0x92                ; fast A20 via the system control port
    or  al, 2
    and al, 0xFE                ; make sure bit 0 stays clear
    out 0x92, al
    ret

; ---------------------------------------------------------------
; Global Descriptor Table: flat model, both segments cover 0-4GB
; ---------------------------------------------------------------
gdt_start:
...
gdt_data:                       ; same, but a writable data segment
    dw 0xFFFF
    dw 0x0000
    db 0x00
...
; ---------------------------------------------------------------
; From here on the CPU is running 32-bit code.
; ---------------------------------------------------------------
bits 32
...
pmode_entry:
    ...
    ; Write directly to VGA text memory. There is no BIOS here:
    ; int 0x10 is gone, so this is the only way to show anything.
    mov edi, 0xB8000
    mov byte [edi],     'P'
